...
function onMove(e){if(!scene.dragStart||!scene.dragMode)return;const dx=e.clientX-scene.dragStart.x,dy=e.clientY-scene.dragStart.y;if(scene.dragMode==="orbit"){scene.yaw=scene.dragStart.yaw+dx*.005;scene.pitch=clamp(scene.dragStart.pitch-dy*.005,-85*DEG,85*DEG);}else if(scene.dragMode==="pan"){if(scene.lockBody)return;const s=(scene.distance/(Math.min(width,height)*.95))*2;scene.focus.x=scene.dragStart.focus.x-dx*s;scene.focus.y=scene.dragStart.focus.y+dy*s;}}
